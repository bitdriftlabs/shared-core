...
pub struct WorkflowDebugStateMap(
  pub(crate) Box<HashMap<WorkflowDebugStateKey, WorkflowDebugState>>,
);
pub type OptWorkflowDebugStateMap = Option<WorkflowDebugStateMap>;

impl WorkflowDebugStateMap {
  #[must_use]
  pub fn into_inner(self) -> HashMap<WorkflowDebugStateKey, WorkflowDebugState> {
    *self.0
  }
}
  ...
  fn merge(&mut self, other: &[WorkflowDebugStateKey], now: OffsetDateTime) {
    for key in other {
      self
        ...
        .or_insert_with(|| WorkflowDebugState {
          count: 1,
