...
fn pair_to_floats(
  ...
  let lhs = if let Some(StringOrFloat::Float(lhs)) =
    resolve_reference(extractions, &pair.lhs, current_log_fields)
  {
    lhs
  } else {
    f64::NAN
  };
  let rhs = if let Some(StringOrFloat::Float(rhs)) =
    resolve_reference(extractions, &pair.rhs, current_log_fields)
  {
    rhs
  } else {
    f64::NAN
