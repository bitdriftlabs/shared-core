...
impl ApplyConfig for LoggerUpdate {
  async fn apply_configuration(
    ...
    // return until the full trigger-buffer config update has been processed by BufferUploadManager,
    // and that path reconciles and replays persisted trigger uploads against the final configured
    // trigger-buffer set. Because BufferProducers are only rebuilt after this await, recovered
    // trigger uploads can re-lock trigger buffers before new thread-local producers are exposed to
    // normal log writing.
