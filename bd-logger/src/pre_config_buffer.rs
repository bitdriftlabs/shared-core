...
use bd_event_buffer::AdmissionContext;
...
#[derive(Debug)]
pub struct PreConfigBuffer<T: ApproximateSize + std::fmt::Debug> {
  max_size: usize,
...
impl<T: ApproximateSize + std::fmt::Debug> PreConfigBuffer<T> {
  ...
  pub fn push(&mut self, entry: T) -> Result<(), Error> {
    let log_size = entry.approximate_size_bytes();
