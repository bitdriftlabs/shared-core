...
fn configuration_caching() {
  ...
  {
    let mut setup = Setup::new_with_directory(directory.clone(), LogMetadata::default().into());
...
fn continuous_buffer_resume_with_full_buffer() {
  ...
  // buffer.
  let mut setup = Setup::new_with_directory(dir, LogMetadata::default().into());
...
fn runtime_caching() {
  ...
  let _setup = Setup::new_with_directory(directory, LogMetadata::default().into());
