...
use bd_proto::protos::client::api::debug_data_request::workflow_state_debug_data::Transition_type;
  ...
  async fn send_debug_data(&mut self) {
    ...
    let _ = self
      ...
      .send(DataUpload::DebugData(DebugDataRequest {
        workflow_debug_data: std::mem::take(&mut self.pending_workflow_debug_state)
          .into_iter()
          .map(|(workflow_id, workflow_debug_data)| WorkflowDebugData {
            workflow_id,
            states: workflow_debug_data
              .into_inner()
              .into_iter()
              .map(|(state, data)| {
                ...
                WorkflowStateDebugData {
                  ...
                  transition_type: Some(match state.transition_type {
                    ...
                    WorkflowDebugTransitionType::Timeout => {
                      Transition_type::TimeoutTransition(true)
                    },
                  }),
