...
// Slice-based wrappers for backward compatibility
pub fn serialize_null_slice(dst: &mut [u8]) -> Result<usize> {
  serialize_null(&mut (&mut *dst))
}

pub fn serialize_boolean_slice(dst: &mut [u8], v: bool) -> Result<usize> {
  serialize_boolean(&mut (&mut *dst), v)
}

pub fn serialize_i64_slice(dst: &mut [u8], v: i64) -> Result<usize> {
  serialize_i64(&mut (&mut *dst), v)
}

pub fn serialize_u64_slice(dst: &mut [u8], v: u64) -> Result<usize> {
  serialize_u64(&mut (&mut *dst), v)
}

pub fn serialize_f32_slice(dst: &mut [u8], v: f32) -> Result<usize> {
  serialize_f32(&mut (&mut *dst), v)
}

pub fn serialize_f64_slice(dst: &mut [u8], v: f64) -> Result<usize> {
  serialize_f64(&mut (&mut *dst), v)
}

pub fn serialize_string_header_slice(dst: &mut [u8], str: &str) -> Result<usize> {
  serialize_string_header(&mut (&mut *dst), str)
}

pub fn serialize_array_begin_slice(dst: &mut [u8]) -> Result<usize> {
  serialize_array_begin(&mut (&mut *dst))
}

pub fn serialize_map_begin_slice(dst: &mut [u8]) -> Result<usize> {
  serialize_map_begin(&mut (&mut *dst))
}

pub fn serialize_container_end_slice(dst: &mut [u8]) -> Result<usize> {
  serialize_container_end(&mut (&mut *dst))
}
