...
fn test_decode_strings() {
    ...
    // Long string
    let long_str = "This is a much longer string that should definitely use the long string encoding mechanism";
    ...
    if let Value::Object(map) = &value {
        if let Some(Value::Array(users)) = map.get("users") {
            ...
            if let Value::Object(alice) = &users[0] {
                assert_eq!(alice.get("name").unwrap(), &Value::String("Alice".to_string()));
...
fn test_boundary_values() {
    ...
    let values = [
        0u64, 1, 100, 255, 256, 65535, 65536,
        u8::MAX as u64, u16::MAX as u64, u32::MAX as u64, u64::MAX,
    ];
...
fn test_accessor_methods() {
    ...
    assert_eq!(obj_val.get("key").unwrap(), &Value::String("value".to_string()));
}
