...
impl<'a> Decoder<'a> {
    ...
    fn decode_value(&mut self) -> Result<Value> {
        ...
        let type_code = deserialize_type_code(remaining).and_then(|(size, code)| {
            self.advance(size);
            Ok(code)
        }).map_err(|e| DeserializationErrorWithOffset::Error(e, self.position))?;
        
        match type_code {
            code if code == TypeCode::Null as u8 => {
                Ok(Value::Null)
            }
            code if code == TypeCode::True as u8 => {
                Ok(Value::Bool(true))
            }
            code if code == TypeCode::False as u8 => {
                Ok(Value::Bool(false))
            }
            code if code == TypeCode::ArrayStart as u8 => {
                self.decode_array()
            }
            code if code == TypeCode::MapStart as u8 => {
                self.decode_object()
            }
            code if code == TypeCode::LongString as u8 => {
                self.decode_long_string()
            }
            code if code >= TypeCode::String as u8 && code <= TypeCode::StringEnd as u8 => {
                self.decode_short_string(code)
            }
            code if code == TypeCode::Float16 as u8  => {
                self.decode_f16()
            }
            code if code == TypeCode::Float32 as u8  => {
                self.decode_f32()
            }
            code if code == TypeCode::Float64 as u8  => {
                self.decode_f64()
            }
            ...
            _ => Err(DeserializationErrorWithOffset::Error(DeserializationError::UnexpectedTypeCode, self.position)),
        }
    }
    ...
    fn decode_f16(&mut self) -> Result<Value> {
        let remaining = self.remaining_data();
        let (size, value) = deserialize_f16_after_type_code(remaining).map_err(|e| DeserializationErrorWithOffset::Error(e, self.position))?;
        self.advance(size);
    ...
    fn decode_f32(&mut self) -> Result<Value> {
        let remaining = self.remaining_data();
        let (size, value) = deserialize_f32_after_type_code(remaining).map_err(|e| DeserializationErrorWithOffset::Error(e, self.position))?;
        self.advance(size);
        Ok(Value::Float(value as f64))
    }
    ...
    fn decode_f64(&mut self) -> Result<Value> {
        let remaining = self.remaining_data();
        let (size, value) = deserialize_f64_after_type_code(remaining).map_err(|e| DeserializationErrorWithOffset::Error(e, self.position))?;
        self.advance(size);
    ...
    fn decode_long_string(&mut self) -> Result<Value> {
        ...
        // let remaining = self.remaining_data();
        let (size, str_slice) = deserialize_long_string_after_type_code(remaining).map_err(|e| DeserializationErrorWithOffset::Error(e, self.position))?;
        self.advance(size);
    ...
    fn decode_short_string(&mut self, type_code: u8) -> Result<Value> {
        let remaining = &self.data[self.position..];
        // let remaining = self.remaining_data();
        let (size, str_slice) = deserialize_short_string_after_type_code(remaining, type_code).map_err(|e| DeserializationErrorWithOffset::Error(e, self.position))?;
        self.advance(size);
        Ok(Value::String(str_slice.to_string()))
    }
    ...
    fn decode_unsigned(&mut self, type_code: u8) -> Result<Value> {
        let remaining = self.remaining_data();
        let (size, value) = deserialize_unsigned_after_type_code(remaining, type_code).map_err(|e| DeserializationErrorWithOffset::Error(e, self.position))?;
        self.advance(size);
    ...
    fn decode_integer(&mut self, type_code: u8) -> Result<Value> {
        let remaining = self.remaining_data();
        let (size, value) = deserialize_signed_after_type_code(remaining, type_code).map_err(|e| DeserializationErrorWithOffset::Error(e, self.position))?;
        self.advance(size);
    ...
    fn decode_array(&mut self) -> Result<Value> {
        ...
        loop {
            ...
            if remaining.is_empty() {
                return Err(DeserializationErrorWithOffset::Error(DeserializationError::UnterminatedContainer, self.position));
            }
            
            let type_code = peek_type_code(remaining).map_err(|e| DeserializationErrorWithOffset::Error(e, self.position))?;
    ...
    fn decode_object(&mut self) -> Result<Value> {
        ...
        loop {
            ...
            let key = match self.decode_value()? {
                Value::String(s) => s,
                _ => return Err(DeserializationErrorWithOffset::Error(DeserializationError::NonStringKeyInMap, self.position)),
            };
