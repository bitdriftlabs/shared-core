...
use bytes::Bytes;
use connect_protocol::is_unary_connect;
...
async fn decode_request<Message: MessageFull>(
...
) -> Result<(HeaderMap<HeaderValue>, Extensions, Message, bool)> {
  ...
  let (message, is_connect) = if is_unary_connect(&parts.headers) {
  ...
  Ok((parts.headers, parts.extensions, message, is_connect))
}
...
async fn unary_connect_handler<OutgoingType: MessageFull, IncomingType: MessageFull>(
  ...
  Ok(
    Response::builder()
      ...
      .body(response.write_to_bytes().unwrap().into())
      .unwrap(),
  )
}
...
pub async fn unary_handler<OutgoingType: MessageFull, IncomingType: MessageFull>(
  ...
  let (headers, extensions, message, is_connect) =
...
async fn server_streaming_handler<ResponseType: MessageFull, RequestType: MessageFull>(
  ...
  tokio::spawn(async move {
    let sender = &mut StreamingApiSender::new(tx, compression, is_connect);
