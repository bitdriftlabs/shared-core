...
impl ResilientKv {
  ...
  pub fn new(buffer: Box<dyn ByteBuffer>) -> anyhow::Result<Self> {
    let mut kv = Self {
      version: 0,
      position: 0,
      buffer,
    ...
    let mut position = 16;
    position += serialize_array_begin(kv.buffer_at_position(position))
  ...
  pub fn from_kv_store(buffer: Box<dyn ByteBuffer>, kv_store: &mut Self) -> anyhow::Result<Self> {
    ...
    position += serialize_map_begin(kv.buffer_at_position(position))
      .map_err(|e| anyhow::anyhow!("Failed to serialize map begin: {e:?}"))?;
    let hashmap = kv_store.as_hashmap()?;
    for (k, v) in &hashmap {
      position += serialize_string(kv.buffer_at_position(position), k)
    ...
    position += serialize_container_end(kv.buffer_at_position(position))
  ...
  fn write_journal_entry(&mut self, key: &str, value: &Value) -> anyhow::Result<()> {
    ...
    position += serialize_map_begin(self.buffer_at_position(position))
      .map_err(|e| anyhow::anyhow!("Failed to serialize map begin: {e:?}"))?;
    position += serialize_string(self.buffer_at_position(position), key)
      .map_err(|e| anyhow::anyhow!("Failed to serialize string key: {e:?}"))?;
    position += encode_into(self.buffer_at_position(position), value)
      .map_err(|e| anyhow::anyhow!("Failed to encode value: {e:?}"))?;
    position += serialize_container_end(self.buffer_at_position(position))
      .map_err(|e| anyhow::anyhow!("Failed to serialize container end: {e:?}"))?;
  ...
  pub fn as_hashmap(&mut self) -> anyhow::Result<HashMap<String, Value>> {
    ...
    serialize_container_end(self.buffer_at_position(self.position))
      .map_err(|e| anyhow::anyhow!("Failed to serialize container end: {e:?}"))?;
    let buffer = self.buffer_at_position(16);
    let decoded: Value =
      decode(buffer).map_err(|e| anyhow::anyhow!("Failed to decode buffer: {e:?}"))?;
