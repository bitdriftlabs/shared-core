...
fn test_memmapped_file_expansion() {
    ...
    for i in 0..5 {
        kv.set(&format!("key_{}", i), &Value::String(format!("val_{}", i))).unwrap();
    }
...
fn test_memmapped_high_water_mark() {
    ...
    // Create with a callback to track high water mark
    static mut HWM_TRIGGERED: bool = false;
    fn hwm_callback(_current: usize, _total: usize, _hwm: usize) {
        unsafe { HWM_TRIGGERED = true; }
    }
...
fn test_memmapped_buffer_usage_ratio() {
    ...
    // Add some data
    for i in 0..5 {
        kv.set(&format!("ratio_key_{}", i), &Value::String(format!("ratio_value_{}", i))).unwrap();
    }
...
fn test_memmapped_large_data_persistence() {
    ...
    // Create first instance with large data set
    {
        ...
        for i in 0..30 {
            kv.set(&format!("user:{}", i), &Value::String(format!("username_{}", i))).unwrap();
        }
        
        // Update some entries
        for i in 0..10 {
            kv.set(&format!("user:{}", i), &Value::String(format!("updated_username_{}", i))).unwrap();
        }
...
fn test_memmapped_concurrent_operations() {
    ...
    for i in 0..50 {
        kv.set(&format!("concurrent_key_{}", i), &Value::Signed(i as i64)).unwrap();
        ...
        if i % 5 == 0 {
            kv.set(&format!("concurrent_key_{}", i / 2), &Value::Bool(true)).unwrap();
        }
