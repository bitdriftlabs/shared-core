...
use crate::{DoubleBufferedKVJournal, InMemoryKVJournal, KVJournal, MemMappedKVJournal};
...
fn test_many_small_entries() -> anyhow::Result<()> {
  ...
  assert!(
    ...
    "Should write at least 100 small entries"
  );
...
fn test_unicode_keys_and_values() -> anyhow::Result<()> {
  ...
  assert_eq!(
    data.get("emoji_🔑"),
    Some(&Value::String("emoji_value_🎉".to_string()))
  );
  assert_eq!(
    data.get("中文键"),
    Some(&Value::String("中文值".to_string()))
  );
    ...
    match db_journal.as_hashmap() {
      Ok(data) => {
        assert_eq!(
          data.len(),
          successful_writes,
          "Should have stored exactly the successful writes"
        );
      },
      Err(e) => {
        // If we can't read the hashmap due to buffer constraints, that's acceptable for this test
        println!(
          "Note: Could not read hashmap due to buffer constraints: {}",
          e
        );
      },
    }
...
fn test_buffer_usage_ratio_accuracy() -> anyhow::Result<()> {
  ...
  assert!(
    initial_ratio > 0.0,
    "Should have some initial usage for metadata"
  );
  ...
  for i in 0 .. 10 {
    ...
    assert!(
      ...
      "Usage ratio should increase with each write"
    );
