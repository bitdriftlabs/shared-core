...
fn test_double_buffered_automatic_compaction_on_high_water_mark() {
  ...
  for round in 0 .. 20 {
    for key_num in 0 .. 3 {
      ...
      let value = format!(
        "data_round_{}_key_{}_with_some_extra_content",
        round, key_num
      );
  ...
  assert_ne!(
    ...
    "Journal should have switched due to high water mark"
  );
  ...
  assert_eq!(
    hashmap.len(),
    3,
    "Should have exactly 3 keys after compaction"
  );
  ...
  for key_num in 0 .. 3 {
    ...
    assert_eq!(
      ...
      "Should have the final value for {}",
      key
    );
  }
}
...
fn test_double_buffered_user_callback_when_both_journals_full() {
  ...
  for i in 0 .. 50 {
    let result = journal.set(
      &format!("key{}", i),
      &Value::String(format!("very_long_value_to_quickly_fill_buffers_{}", i)),
    );
    if result.is_ok() {
      successful_writes += 1;
    } else {
  ...
  let _final_result = journal.set(
    "final_key",
    &Value::String("final_value_that_will_trigger_compaction_failure".to_string()),
  );

  // Verify the high water mark flag is set when compaction couldn't resolve the issue
  assert!(
    journal.is_high_water_mark_triggered(),
    "High water mark should be triggered when both journals are full"
  );
}
