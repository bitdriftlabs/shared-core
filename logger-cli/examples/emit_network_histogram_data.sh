...
SLEEP_MS=${SLEEP_MS:-0}
VERBOSE=${VERBOSE:-1}
BUCKET_STRESS=${BUCKET_STRESS:-0}

methods=(GET POST PUT DELETE PATCH)
hosts=(api.bitdrift.dev api.bitdrift.io api.example.com uploads.example.com)
paths=(/v1/sessions /v1/logs /v1/ingest /v1/metrics /v1/alerts /v2/query /v3/workflows)
...
rand_wide() {
  local min=$1
  local max=$2
  if [[ "$BUCKET_STRESS" -eq 1 ]]; then
    if [[ $((RANDOM % 4)) -eq 0 ]]; then
      echo $((min + RANDOM % (max * 20)))
    else
      echo $((min + RANDOM % max))
    fi
  else
...
emit_request() {
  ...
  local status=$?
  if [[ "$status" -ne 0 ]]; then
    echo "[error] logger-cli request failed with exit code $status" >&2
    exit "$status"
  fi
}
...
emit_response() {
  ...
  local status=$?
  if [[ "$status" -ne 0 ]]; then
    echo "[error] logger-cli response failed with exit code $status" >&2
    exit "$status"
  fi
}
...
for _ in $(seq 1 "$ITERATIONS"); do
  ...
  req_bytes=$(rand_wide 200 20000)
  resp_bytes=$(rand_wide 500 300000)
  duration_ms=$(rand_wide 5 9000)
  ttfb_ms=$(rand_wide 1 1500)
  dns_ms=$(rand_wide 1 120)
  connect_ms=$(rand_wide 1 300)
  tls_ms=$(rand_wide 1 250)
  ...
  emit_request "$span_id" "$method" "$host" "$path" "$req_bytes" "$content_type" "Android"
  emit_response "$span_id" "$method" "$host" "$path" "$status" "$duration_ms" "$resp_bytes" "$ttfb_ms" "$dns_ms" "$connect_ms" "$tls_ms" "$error_type" "Android"
