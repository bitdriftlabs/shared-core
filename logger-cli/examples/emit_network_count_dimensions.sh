...
# Emits a large number of distinct network-response dimensions to stress TopK/group-by charts.
# Designed to match the default "Network Response" workflow events (span/HTTPResponse).
#
# Usage (after logger-cli start):
#   LOGGER_CLI_BIN=logger-cli ITERATIONS=400 DISTINCT_PATHS=250 ./emit_network_count_dimensions.sh

LOGGER_CLI_BIN=${LOGGER_CLI_BIN:-logger-cli}
ITERATIONS=${ITERATIONS:-400}
SLEEP_MS=${SLEEP_MS:-0}
VERBOSE=${VERBOSE:-1}
DISTINCT_PATHS=${DISTINCT_PATHS:-250}
DISTINCT_HOSTS=${DISTINCT_HOSTS:-6}
DISTINCT_APPS=${DISTINCT_APPS:-4}
...
build_apps() {
  local apps=()
  local i=0
  while [[ ${#apps[@]} -lt "$DISTINCT_APPS" ]]; do
    apps+=("io.mart.test.${i}")
    i=$((i + 1))
  done
  echo "${apps[@]}"
